...
{
  if(!in.is_string())
    throw "symbol_from_json: expected string for key '"+key+"'";
  return in.value;
}
...
{
  if(!in.is_object())
    throw "symbol_from_json takes an object";
  symbolt result;
  ...
  for(const auto &kv : in.object)
  {
    ...
    {
      irept irep;
      json2irep.convert_from_json(kv.second, irep);
      if(kv.first=="type")
      ...
      throw "symbol_from_json: unexpected key '"+kv.first+"'";
  }
