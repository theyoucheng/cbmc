                                ...
                                const lpoints_valuet& value)
{
  ...
  //assumptions.push_back(failed->guard_literal);
  ...
  if(bmc.prop_conv()==decision_proceduret::D_SATISFIABLE)
  {
    for(const auto &l : lpoints)
	{
	  status() << bmc.prop_conv.l_get(l.first).is_true() << " " ;
	}
    status() << eom;
    return false;
  }
...
void fault_localizationt::run(irep_idt goal_id)
{
 // find failed property
  ...
  status() << "source location: " << failed->source.pc->source_location << eom << eom;
...
{
  status() << "goal covered, goal map size: " << goal_map.size() << eom << eom;
  for(auto &g : goal_map)
  {
    status() << g.first << ", " << g.second.status_string() << eom;
  }
  for(auto &g : goal_map)
  {
    // failed already?
    ...
    for(auto &c : g.second.instances)
    {
      ...
      //if(solver.l_get(cond).is_false())
      {
        ...
        //localize faults
        status() << eom << eom << "+++++++++++++++++++ pfl start ++++++++++++++++++++ "<< eom;

        run(g.first);
        status() << eom << "+++++++++++++++++++ pfl end ++++++++++++++++++++ "<< eom << eom;
