...
protected:
  ...
  void ranged_type_check(const exprt &expr, const guardt &guard);
...
void goto_checkt::ranged_type_check(
  const exprt &expr,
  const guardt &guard)
{
  ...
  if(real_type.id()!=ID_signedbv)
    return;
  const exprt &lower_bound=static_cast<const exprt &>(real_type.find("lower_bound"));
  const exprt &upper_bound=static_cast<const exprt &>(real_type.find("upper_bound"));
